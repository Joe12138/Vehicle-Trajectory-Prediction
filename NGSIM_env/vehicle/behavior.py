...
class IDMVehicle(ControlledVehicle):
    ...
    def step(self, dt):
        """
        Step the simulation.
        Increase a timer used for decision policies, and step the vehicle dynamics.
    ...
    def change_lane_policy(self):
        ...
        if self.lane_index != self.target_lane_index:
            ...
            if self.lane_index[:2] == self.target_lane_index[:2]:
                for v in self.road.vehicles:
                    if v is not self and v.lan_index != self.target_lane_index and isinstance(v, ControlledVehicle) and\
                            v.target_lane_index == self.target_lane_index:
        ...
        for lane_index in self.road.network.side_lanes(self.lane_index):
            # Is te candidate lane close enough ?
    ...
    def acceleration(self, ego_vehicle, front_vehicle=None, rear_vehicle=None):
        ...
        :param ego_vehicle: the vehicle whose desired acceleration is to be computed. It does not have to be an IDM
                           vehicle, which is why this method is a class method. This allows an IDM vehicle to reason
                           about other vehicles behaviors even though they may not IDMs.
        ...
        acceleration = self.COMFORT_ACC_MAX * (1-np.power(max(ego_target_velocity, 0)/ ego_target_velocity, self.DELTA))

        if front_vehicle:
            d = ego_vehicle.lane_distance_to(front_vehicle)
            acceleration -= self.COMFORT_ACC_MAX * np.power(self.desired_gap(ego_vehicle, front_vehicle) /
                                                            utils.not_zero(d), 2)
        return acceleration

    def mobil(self, lane_index):
        """
        MOBIL lane change model: Minimizing Overall Breaking Induced by a Lane change

        The vehicle should change lane only if:
        - after changing it (and/or following vehicles) can accelerate more;
        - it doesn't impose an unsafe breaking on its new following vehicle.
        ...
        if new_following_pred_a < -self.LANE_CHANGE_MAX_BREAKING_IMPOSED:
            return False
        ...
        if self.route and self.route[0][2]:
            ...
            elif self_pred_a < -self.LANE_CHANGE_MAX_BREAKING_IMPOSED:
                return False
        ...
        # Is there an acceleration advantage for me and/or my followers to change lane?
            ...
            jerk = self_pred_a-self_a+self.POLITENESS*(new_following_pred_a-new_following_a+old_following_pred_a-
                                                       old_following_a)
    ...
    def desired_gap(self, ego_vehicle, front_vehicle=None):
        ...
        ab = -self.COMFORT_ACC_MAX * self.COMFORT_AAC_MIN
    ...
    def maximum_velocity(self, front_vehicle=None):
        ...
        Assume the front vehicle is going to brake at full deceleration and that it will be noticed after a given delay,
        and compute the maximum velocity which allows the ego-vehicle to brake enough to avoid the collision.
        ...
        a0 = self.COMFORT_AAC_MIN
        a1 = self.COMFORT_AAC_MIN  # Maybe COMFORT_ACC_MAX here?
        tau = self.TIME_WANTED
...
class LinearVehicle(IDMVehicle):
    ...
    def __init__(self, road, position, heading=0, velocity=0, target_lane_index=None, target_velocity=None, route=None,
                 enable_lane_change=True, timer=None):
        super(LinearVehicle, self).__init__(road, position, heading, velocity, target_lane_index, target_velocity,
    ...
    def acceleration(self, ego_vehicle, front_vehicle=None, rear_vehicle=None):
        ...
        :param ego_vehicle: the vehicle whose desired acceleration is to be computed. It does not have to be an Linear
                            vehicle, which is why this method is a class method. This allows a Linear vehicle to reason
                            about other vehicles behaviors even though they may not Linear.
        :param front_vehicle: the vehicle preceding the ego-vehicle
        :param rear_vehicle: the vehicle following the ego-vehicle
        :return: the acceleration command for the ego-vehicle [m/s2]
         """
