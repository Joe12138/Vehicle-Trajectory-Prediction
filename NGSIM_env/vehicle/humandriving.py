...
class NGSIMVehicle(IDMVehicle):
    ...
    def act(self, action=None):
        ...
        self.check_collision(front_vehicle)  # should be a method
        self.action = action
    ...
    def check_collision(self, other):
        ...
        # if both vehicles are NGSIM vehicles and have not been overridden
        if isinstance(self, NGSIMVehicle) and not self.overtaken and isinstance(other, NGSIMVehicle) \
                and not other.overtaken:
            return

        # Accurate rectangular check
        if utils.rotated_rectangles_intersect((self.position, 0.9*self.LENGTH, 0.9*self.WIDTH, self.heading),
                                              (other.position, 0.9*other.LENGTH, 0.9*other.WIDTH, other.heading)) and \
            self.appear:
...
class HumanLikeVehicle(IDMVehicle):
    ...
    def create(cls, road, vehicle_ID, position, v_length, v_width, ngsim_traj, heading=0, velocity=0, acc=0,
        ...
        """
        v = cls(road, position, heading, velocity, acc, target_velocity=target_velocity, vehicle_ID=vehicle_ID,
                v_length=v_length, v_width=v_width, ngsim_traj=ngsim_traj, human=human, IDM=IDM)
        return v
    ...
    def steering_control(self, trajectory, step):
        ...
        4. Heading rate command command is convert to a steering angle
