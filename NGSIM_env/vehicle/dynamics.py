...
class Vehicle(Loggable):
    ...
    def step(self, dt):
        ...
        Integrate a modifiied bicycle model with a 1st-order response on the steering wheel dynamics
        If the vehicle is crashed, the actions are overridden with erratic steering and braking until complete stop
        The vehicle's current lane is updated

        :param dt: timestep of intergration of the model [s]
        """
        if self.crashed:
    ...
    def check_collision(self, other):
        ...
        if NGSIM_env.utils.rotated_rectangles_intersect((self.position, 0.9*self.LENGTH, 0.9*self.WIDTH, self.heading),
    ...
    def dump(self):
        ...
        if self.road:
            ...
            front_vehicle, rear_vehicle = self.road.neightbour_vehicles(self)
            if front_vehicle:
                data.update({
...
class Obstacle(Vehicle):
    ...
    def __init__(self, road, position, heading=0):
        super(Obstacle, self).__init__(road, position, heading=heading, velocity=0)
