...
class GrayscaleObservation(ObservationType):
    """
    An observation class that collects directly what the simulator renders as the input, and stacks the collected frames
    just as in the nature DQN. Specific keys are expected in the configuration dictionary passed.
...
class TimeToCollisionObservation(ObservationType):
    ...
    def observe(self):
        ...
        obs_velocity = 3
        v0 = grid.shape[0]+self.env.vehicle.velocity_index-obs_velocity//2
        vf = grid.shape[0]+self.env.vehicle.velocity_index+obs_velocity//2
...
class KinematicObservation(ObservationType):
    ...
    def __init__(self, env, features=FEATURES, vehicles_count=6, features_range=None, absolute=False, normalize=True,
                 clip=True, order="sorted", observe_intentions=False, **kwargs):
        """
    ...
    def observe(self):
        ...
        close_vehicles = self.env.road.close_vehicle_to(self.env.vehicle, self.env.PERCEPTION_DISTANCE,
        ...
        if close_vehicles:
            ...
            df = df.append(pandas.DataFrame.from_records(
                [v.to_dict(origin, observe_intentions=self.observe_intentions) for v in close_vehicles[
...
class OccupancyGridObservation(ObservationType):
    ...
    def __init__(self, env, features=FEATURES, grid_size=GRID_SIZE, grid_step=GRID_STEP, features_range=None,
    ...
    def space(self):
        try:
            ...
            return spaces.Dict(dict(
                ...
                obsercation=spaces.Box(-np.inf, np.inf, shape=obs["observation"].shape, dtype=np.float32),
            ))
        except AttributeError:
            return None
