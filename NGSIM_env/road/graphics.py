...
class LaneGraphics(object):
    ...
    """
    STRIPE_SPACING = 5  # Offset between stripes [m]
    STRIPE_LENGTH = 3  # Length of a stripe [m]
    STRIPE_WIDTH = 0.3  # Width of a stripe
    ...
    def continuous_line(cls, lane, surface, stripes_count, s0, side):
        ...
        :param side: which side of the road to draw [0: left, 1:right]
        """
        starts = [s0+0*cls.STRIPE_SPACING]  # something weird here
    ...
    def draw_stripes(cls, lane, surface, starts, ends, lats):
        ...
        :param starts: a list of starting longitudinal position for each stripe [m]
        :param ends: a list of ending longitudinal position for each stripe [m]
        :param lats: a list of lateral positions for each stripes [m]
        """
        starts = np.clip(starts, 0, lane.length)
        ends = np.clip(ends, 0, lane.length)
        for k in range(len(starts)):
            if abs(starts[k]-ends[k]) > 0.5*cls.STRIPE_LENGTH:
                pygame.draw.line(surface, surface.WIDTH,
